...
- `verifyvat audit`: Reads recent local audit records from SQLite, renders them in a human-readable table, may filter the selected rows by exact status or case-insensitive search, and may export the selected rows as CSV or JSON. It must not perform remote verification requests.
- `verifyvat discovery`: Lists supported formats and/or source registries through the SDK discovery endpoints, supports `--country`, `--region`, `--formats`, `--sources`, and `--json`, and must not mutate the audit database.
...
| verifyvat audit | Reads recent local audit history, optionally filters the selected rows, and exports them as CSV or JSON. | --limit [INT], --export-csv [FILE_PATH], --status [STATUS], --search [TEXT], --json |
